"""
Combined Entropy-Adaptive Eviction + TurboQuant KV Cache Compression for Qwen3.5-9B
======================================================================================
Tests three compression approaches on Qwen3.5-9B (GPU, eager attention):
...
def print_results_table(results: dict):
    ...
    print(f"\n{'=' * 110}")
    print(f"COMBINED COMPRESSION RESULTS — Qwen3.5-9B")
    print(f"{'=' * 110}")
...
def main():
    ...
    print(f"Qwen3.5-9B — GPU (bf16) — GQA-aware")
    print(f"{'=' * 70}")
    ...
    model = Qwen3_5ForCausalLM.from_pretrained(
        ...
        device_map="auto",  # 9B won't fit fully on 12GB GPU
    ...
    # Load pre-computed head entropies (try 9B-specific, fall back to 4B)
    print(f"\n[{timestamp()}] Loading entropy config...")
    try:
        head_entropies = load_entropy_config()
    except FileNotFoundError:
        print("  9B entropy config not found, using 4B config (same architecture)")
        ENTROPY_CONFIG_PATH_4B = PROJECT_ROOT / "models" / "qwen3.5-4b" / "entropy_config.json"
        with open(ENTROPY_CONFIG_PATH_4B, 'r') as f:
            config_data = json.load(f)
        head_entropies = {}
        for entry in config_data.get('head_entropies', config_data.get('heads', [])):
            layer_idx = entry.get('layer', entry.get('layer_idx'))
            head_idx = entry.get('head', entry.get('head_idx'))
    ...
    # Save results
    results_path = RESULTS_DIR / 'qwen35_9b_combined_results.json'
    serializable = {}
