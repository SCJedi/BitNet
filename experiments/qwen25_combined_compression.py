"""
Combined Entropy-Adaptive Eviction + TurboQuant KV Cache Compression for Qwen2.5-7B-Instruct
==============================================================================================
Tests three compression approaches on Qwen2.5-7B-Instruct (GPU, eager attention):
...
Key architecture differences from Qwen3.5-4B experiment:
  - ALL 28 layers have standard attention (no hybrid / linear attention layers)
  - GQA: 28 query heads, 4 KV heads (7:1 ratio, more aggressive than 4:1)
  - head_dim=128 (half of Qwen3.5's 256)
  - Standard Qwen2 architecture (model_type="qwen2")
  - Compression applies to ALL layers (not just 8/32)
  - Effective compression ratio = raw compression ratio (no hybrid discount)
...
Hardware: CUDA (bf16 or 4-bit quantized weights via bitsandbytes)
  - Model is ~14GB in bf16, so we try bitsandbytes 4-bit loading first
  - Falls back to bf16 with device_map="auto" if bitsandbytes unavailable
"""
...
# Architecture constants — Qwen2.5-7B-Instruct
FULL_ATTENTION_LAYERS = set(range(28))  # ALL 28 layers are standard attention
N_LAYERS = 28
N_FULL_ATTN_LAYERS = 28
N_QUERY_HEADS = 28
N_KV_HEADS = 4
GQA_RATIO = N_QUERY_HEADS // N_KV_HEADS  # 7
HEAD_DIM = 128
...
class TurboQuantVectorQuantizer:
    ...
    def compress(self, vectors: torch.Tensor) -> dict:
        ...
        Uses per-vector normalization to handle high-variance keys (e.g., post-RoPE keys
        with std=27+ in Qwen2.5). Each vector is divided by its L2 norm before quantization,
        then the norm is stored separately for reconstruction.
        """
        n_tokens, dim = vectors.shape
        assert dim == self.dim

        # Stage 1: Convert and normalize per-vector
        vectors_f32 = vectors.float().cpu()
        vec_norms = vectors_f32.norm(dim=-1, keepdim=True).clamp(min=1e-8)  # (n_tokens, 1)
        vectors_normed = vectors_f32 / vec_norms  # unit vectors

        # Rotate the normalized vectors
        rotated = self.rotation.rotate(vectors_normed)

        # Estimate sigma from normalized+rotated data (should be ~1/sqrt(dim) for unit vectors)
        ...
        # Denormalize stage1 reconstruction
        stage1_recon_denorm = stage1_recon * vec_norms
        ...
        result = {
            ...
            'n_bits': self.n_bits,
            'n_tokens': n_tokens,
            'vec_norms': vec_norms,  # stored for decompress
        }

        # Stage 2: QJL residual correction (on denormalized vectors)
        if self.use_qjl:
            residual = vectors_f32 - stage1_recon_denorm
            ...
            result['stage1_recon'] = stage1_recon_denorm
        else:
            result['stage1_recon'] = stage1_recon_denorm
    ...
    def decompress(self, compressed: dict) -> torch.Tensor:
        """Decompress back to full vectors (stage 1 reconstruction + denormalization)."""
        indices = compressed['indices']
        sigma = compressed['sigma']
        n_tokens = compressed['n_tokens']
        vec_norms = compressed['vec_norms']  # (n_tokens, 1)
        ...
        recon_normed = self.rotation.inverse_rotate(recon_rotated)
        return recon_normed * vec_norms  # denormalize
...
###############################################################################
# INLINE ENTROPY COMPUTATION (no pre-computed config file for Qwen2.5-7B)
###############################################################################

def compute_head_entropies_inline(model, tokenizer, calibration_texts,
                                  max_tokens=128, num_texts=3) -> Dict:
    """
    Compute per-query-head entropy from a few forward passes.
    Runs baseline (no compression) forward passes and collects attention weights
    from ALL 28 layers.

    Returns dict mapping (layer_idx, query_head_idx) -> entropy value.
    """
    print(f"  [{timestamp()}] Computing head entropies inline from {num_texts} texts...")

    # We need to temporarily hook attention to collect weights
    collected_weights = defaultdict(list)  # layer_idx -> list of attn_weight tensors
    ...
    def make_hook(layer_idx):
        def hook_fn(module, input, output):
            ...
            if isinstance(output, tuple) and len(output) >= 2 and output[1] is not None:
                # attn_weights: (batch, n_query_heads, seq, seq)
                aw = output[1].detach().float().cpu()
                collected_weights[layer_idx].append(aw)
        return hook_fn
    ...
    # Run forward passes with output_attentions=True
    texts_to_use = calibration_texts[:num_texts]
    for text_idx, text in enumerate(texts_to_use):
    ...
    head_entropies = {}
    for layer_idx in range(N_LAYERS):
        if layer_idx not in collected_weights or not collected_weights[layer_idx]:
            # If no weights collected for this layer, set uniform entropy
            for h in range(N_QUERY_HEADS):
                head_entropies[(layer_idx, h)] = 1.0
            continue

        # Average over all texts
        all_aw = torch.cat(collected_weights[layer_idx], dim=0)  # (n_texts, n_heads, seq, seq)

        for h in range(N_QUERY_HEADS):
            # Compute entropy of attention distribution for this head
            aw_h = all_aw[:, h, :, :]  # (n_texts, seq, seq)
            # Clamp to avoid log(0)
            aw_h = aw_h.clamp(min=1e-10)
            # Shannon entropy: -sum(p * log2(p)) averaged over positions and texts
            entropy = -(aw_h * aw_h.log2()).sum(dim=-1)  # (n_texts, seq)
            mean_entropy = entropy.mean().item()
            if mean_entropy > 0:
                head_entropies[(layer_idx, h)] = mean_entropy
            else:
                head_entropies[(layer_idx, h)] = 1.0

    # Clean up collected weights
...
def mask_entropy_adaptive_gqa(attn_weights, keep_ratio, head_entropies=None,
    ...
    attn_weights: (n_query_heads=28, seq_len, seq_len) - post-softmax weights

    GQA-aware eviction for Qwen2.5-7B (7:1 ratio):
    - Compute entropy per query head
    - For each KV head group (7 query heads sharing 1 KV head):
    ...
    Group 0: query heads 0-6  -> KV head 0
    Group 1: query heads 7-13 -> KV head 1
    Group 2: query heads 14-20 -> KV head 2
    Group 3: query heads 21-27 -> KV head 3
    """
...
def _quantize_kv_layer(key_states: torch.Tensor, value_states: torch.Tensor,
    ...
    key_states, value_states: (batch, n_kv_heads=4, seq_len, head_dim=128)
...
def patched_eager_attention_forward(module, query, key, value, attention_mask,
    ...
    """
    Modified eager attention for Qwen2.5-7B with eviction and/or quantization.

    Signature matches Qwen2's eager_attention_forward exactly:
      module: nn.Module with .layer_idx, .num_key_value_groups
      query: (batch, n_heads=28, seq, head_dim=128)
      key: (batch, n_kv_heads=4, seq, head_dim=128)
      value: (batch, n_kv_heads=4, seq, head_dim=128)
    ...
    # Compress all layers when active (ALL 28 layers have standard attention)
    if state['active'] and layer_idx in FULL_ATTENTION_LAYERS:
        # Quantize BEFORE repeat_kv (operates on 4 KV heads, not 28)
        if state['mode'] in ('quantization', 'combined', 'entropy_quant'):
            key, value = _quantize_kv_layer(key, value, layer_idx)

    # Expand KV heads for attention (GQA: 4 -> 28)
    ...
    # Eviction (after softmax, operates on 28 query heads)
    if state['active'] and layer_idx in FULL_ATTENTION_LAYERS:
        if state['mode'] in ('eviction', 'combined'):
            keep_ratio = state['keep_ratio']
            aw = attn_weights[0]  # (28, seq, seq)
...
def install_patch():
    """Install the monkey-patched attention forward into Qwen2 module."""
    ...
    print("  Attention patch installed for Qwen2.5")
...
def compute_compression_ratio(mode: str, keep_ratio: float, n_bits: int) -> float:
    """
    Compute compression ratio for Qwen2.5-7B.
    Since ALL 28 layers have standard attention, the effective compression ratio
    equals the raw per-layer ratio (no hybrid discount needed).
    ...
    # ALL layers are compressible, so effective = raw ratio
    return layer_ratio
...
def get_experiment_configs():
    ...
    # 2-3. Eviction configs SKIPPED (already proven BLEU=1.0 in prior run)
    # eviction_2x: BLEU=1.000, PPL_ratio=1.258
    # eviction_3x: BLEU=1.000, PPL_ratio=1.455
...
def run_experiment(model, tokenizer, sequences, head_entropies, configs,
    ...
    for cfg_idx, cfg in enumerate(configs):
        ...
        # ALL layers are standard attention, so effective = raw ratio
...
def print_results_table(results: dict):
    ...
    print(f"\n{'=' * 110}")
    print(f"COMBINED COMPRESSION RESULTS — Qwen2.5-7B-Instruct")
    print(f"{'=' * 110}")
    ...
    # Qwen2.5-7B specific notes
    print(f"\nNote: ALL 28 layers compressed (no hybrid discount).")
    print(f"  GQA ratio: 7:1 (28 query heads, 4 KV heads)")
...
def load_model_and_tokenizer():
    """
    Load Qwen2.5-7B-Instruct with appropriate quantization for 12GB GPU.
    Tries bitsandbytes 4-bit first, falls back to 8-bit, then bf16.
    """
    from transformers import AutoModelForCausalLM, AutoTokenizer

    model_name = "Qwen/Qwen2.5-7B-Instruct"

    # Try bitsandbytes 4-bit loading first (model is ~14GB in bf16)
    try:
        from transformers import BitsAndBytesConfig
        print(f"  Attempting 4-bit quantized loading via bitsandbytes...")
        ...
        print(f"  Loaded with 4-bit quantization (bitsandbytes)")
    except ImportError:
        print(f"  bitsandbytes not available, trying bf16 with device_map='auto'...")
        try:
            model = AutoModelForCausalLM.from_pretrained(
                model_name,
            ...
            print(f"  Loaded in bf16 with device_map='auto'")
        except Exception as e:
            print(f"  bf16 loading failed: {e}")
            print(f"  Trying float16...")
            model = AutoModelForCausalLM.from_pretrained(
                model_name,
            ...
            print(f"  Loaded in float16 with device_map='auto'")
...
def main():
    ...
    print(f"Qwen2.5-7B-Instruct — GPU — GQA-aware (7:1)")
    print(f"{'=' * 70}")
    ...
    # Load model
    print(f"\n[{timestamp()}] Loading Qwen2.5-7B-Instruct...")
    ...
    # Compute head entropies inline (no pre-computed config for Qwen2.5-7B)
    print(f"\n[{timestamp()}] Computing head entropies inline...")
    head_entropies = compute_head_entropies_inline(
        model, tokenizer, CALIBRATION_TEXTS,
        max_tokens=128, num_texts=3
    )
    ent_values = list(head_entropies.values())
    print(f"  Computed {len(head_entropies)} head entropies (all {N_LAYERS} layers)")
    ...
    # 28 layers x 4 KV heads x 4 bit widths (2,3,4,5) = 448 objects
    ...
    # Save results
    results_path = RESULTS_DIR / 'qwen25_combined_results.json'
    serializable = {}
    ...
    output = {
        'experiment': 'qwen2.5_7b_combined_entropy_turboquant',
